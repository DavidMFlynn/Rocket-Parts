...
//  ***** for STL output *****
//
// rotate([180,0,0]) Stager_Cup(Tube_OD=BT75Body_OD, ID=BT54Body_ID, nLocks=3, BoltsOn=true); // a.k.a. Sustainer Motor Retainer
// rotate([-90,0,0]) Stager_LockRod(Adj=0.5);
// rotate([-90,0,0]) Stager_LockRod(Adj=0.0);
//
// Stager_Saucer(Tube_OD=BT75Body_OD, nLocks=3, HasElectrical=false); // Bolts on
//
// Stager_LockRing(Tube_OD=BT75Body_OD, nLocks=3, FlexComp_d=0.0); 
// Stager_LockStop(Tube_OD=BT75Body_OD, HasMagnet=true);
// Stager_LockStop(Tube_OD=BT75Body_OD, HasMagnet=false);
//
// Stager_Mech(Tube_OD=BT75Body_OD, nLocks=3, Skirt_ID=BT75Body_ID, Skirt_Len=30);
//
...
//  ***** Routines *****
//
// Stager_CupHoles(Tube_OD=BT75Body_OD, ID=Default_ID, nLocks=3, BoltsOn=true);
// Stager_SaucerBoltPattern(Tube_OD=BT75Body_OD, nLocks=3);
...
//
// ShowStager();
// ShowStagerAssy(Tube_OD=BT75Body_OD, Tube_ID=BT75Body_ID, nLocks=3, ShowLocked=true);
// ShowStagerAssy(Tube_OD=BT75Body_OD, Tube_ID=BT75Body_ID, nLocks=3, ShowLocked=false);
//
// ***********************************
...
function BearingBallCircle_d(Tube_OD=BT75Body_OD)=Tube_OD-6-Ball_d;
function Race_ID(Tube_OD=BT75Body_OD)=BearingBallCircle_d(Tube_OD=Tube_OD)-Ball_d-Bolt4Inset*4;
...
module ShowStager(Tube_OD=BT75Body_OD, Tube_ID=BT75Body_ID, nLocks=3){
	...
	Stager_Mech(Tube_OD=Tube_OD, nLocks=nLocks, Skirt_ID=Tube_ID, Skirt_Len=30);
...
module ShowStagerAssy(Tube_OD=BT75Body_OD, Tube_ID=BT75Body_ID, nLocks=3, ShowLocked=true){
	...
	translate([0,0,50]) Stager_Cup(Tube_OD=Tube_OD, ID=BT54Body_ID, nLocks=3, BoltsOn=true);
	
	translate([0,0,1]) rotate([0,0,Lock_a]) Stager_LockRing(Tube_OD=Tube_OD, nLocks=nLocks);

	//*
	translate([0,0,-Race_Z]) 
	difference(){
		Stager_Mech(Tube_OD=Tube_OD, nLocks=nLocks, Skirt_ID=Tube_ID, Skirt_Len=30, ShowLocked=ShowLocked);
...
module Stager_ServoPlate(Tube_OD=BT75Body_OD, Skirt_ID=BT75Body_ID){
	...
	module StopBlock(LockedBlock=true){
		...
		difference(){
			...
			translate([BlockOffset,CablePath_Y,5.5+Magnet_d/2]) rotate([0,90,0]) cylinder(d=Magnet_d, h=Block_T+1, center=true);
	...
	difference(){
		union(){
			cylinder(d=Skirt_ID, h=5);
			
			// Servo base
			translate([-8,0,-5]) cube([54,25,10],center=true);
		...
		// Servo
		rotate([0,0,180]) translate([18,0,-10]) Servo_LD20MG(BottomMount=true,TopAccess=false);
...
module Stager_LockRing(Tube_OD=BT75Body_OD, nLocks=3, FlexComp_d=0.0){
	...
	difference(){
		union(){
			...
			cylinder(d=OD, h=Tube_Len+3, $fn=$preview? 90:360);
			
		} // union
		
		//translate([0,0,Ball_Z]) rotate_extrude() translate([Locked_Ball_Y-3,0,0]) circle(d=LockBall_d+IDXtra*2);
		//translate([0,0,Ball_Z]) rotate_extrude() translate([Locked_Ball_Y,0,0]) circle(d=LockBall_d+6);
		...
		translate([0,0,Tube_Len-LockBall_d-9-Overlap]) cylinder(d1=Large_ID, d2=Small_ID, h=8); // transition
		//translate([0,0,Tube_Len]) cylinder(d1=Small_ID, d2=OD-8, h=6+Overlap);
	...
	difference(){
		union(){
			...
			hull() {
				...
				translate([0,BC_r,0]) cylinder(d=10, h=Plate_H);
			} // hull
...
module Stager_CupHoles(Tube_OD=BT75Body_OD, ID=Default_ID, nLocks=3, BoltsOn=true, Offset_a=0){
	Collar_h=18;
	nBolts=nLocks*4;
...
module Stager_Cup(Tube_OD=BT75Body_OD, ID=Default_ID, nLocks=3, BoltsOn=true, Collar_h=17, Offset_a=0){
...
module Stager_SaucerBoltPattern(Tube_OD=BT75Body_OD, nLocks=3){
	Inset_Y=7.5;
...
module Stager_LockRod_Holes(Tube_OD=BT75Body_OD, nLocks=3, Offset_a=0){
	LR_X=Stager_LockRod_X;
	LR_Y=Stager_LockRod_Y;
	LR_Z=Stager_LockRod_Z;
...
module Stager_Saucer(Tube_OD=BT75Body_OD, nLocks=3){
	Len=Saucer_Len;
...
module Stager_BallSpacer(Tube_OD=BT75Body_OD){
	Thickness=1.5;
...
module Stager_Mech(Tube_OD=BT75Body_OD, nLocks=3, Skirt_ID=BT75Body_ID, Skirt_Len=30, ShowLocked=true){
	...
	difference(){
		...
		union(){
			...
			translate([0,0,Race_Z-Race_W-Skirt_Len]) 
				Tube(OD=Tube_OD, ID=Skirt_ID, Len=Skirt_Len+Overlap*2, myfn=$preview? 90:360);
			
			//translate([0,0,Race_Z-23]) 
				//Tube(OD=Tube_OD-1, ID=Skirt_ID-5.2, Len=13, myfn=$preview? 90:360);
...
module Stager_ArmDisarmAccess(Tube_OD=BT75Body_OD, Len=BT75Body_OD){
	...
	rotate([0,0,-Cable_Offset_a-66]) translate([0,BearingBallCircle_d(Tube_OD=Tube_OD)/2-2,Race_Z-Race_W-3])
