// ************************************************************************************
// involute gear library
//
// Filename: involute_gears.scad
// Modified and expanded by: David M. Flynn
// Revision: 1.0.2 9/7/2025
// Units: mm
// ************************************************************************************
//  ***** Notes *****
//
...
//  ***** History *****
//
echo("involute_gears.scad 1.0.2");
// 1.0.2 9/7/2025  Code cleanup, worked on bevel gear
// 1.0.1 3/12/2020 Added this header, some comments and functions.
// 1.0   3/12/2020 First rev'd version, copied from Parametric Involute Bevel and Spur Gears by GregFrost
//
// ************************************************************************************
//  ***** Routines *****
//
// function GetPitch_Radius(nTeeth, Circular_Pitch)
// function GetPitch_Apex(Gear1_Pitch_r, Gear2_Pitch_r, Axis_a)
// function GetCone_Dist(Pitch_Apex, Pitch_r)
// function GetCone_Distance(nGear1Teeth, nGear2Teeth, Circular_Pitch, axis_angle)
// function GetPitch_Apex2(cone_distance, Gear2_Pitch_r)
// function GetPitch_a(Pitch_r, Cone_Dist)
//
// Bevel Gear Finishing Options:
//  bevel_gear_flat = 0;
//  bevel_gear_back_cone = 1;
//
// bevel_gear(number_of_teeth=11, cone_distance=100, face_width=20, outside_circular_pitch=1000, pressure_angle=30, clearance = 0.2,
//					bore_diameter=5, gear_thickness = 15, backlash = 0, involute_facets=0, finish = -1);
//
// gear(number_of_teeth=15, circular_pitch=false, diametral_pitch=false,
//	pressure_angle=28, clearance = 0.2, gear_thickness=5, rim_thickness=8, rim_width=5,
//	hub_thickness=10, hub_diameter=15, bore_diameter=5, circles=0, backlash=0, twist=0, involute_facets=0, flat=false);
//
// ************************************************************************************
//  ***** Notes *****
//
...
// test_bevel_gear();
//
// bevel_gear_pair(gear1_teeth = 51, gear2_teeth = 13, axis_angle = 90, outside_circular_pitch=500);
//
// ************************************************************************************
...
function GetPitch_Radius(nTeeth, Circular_Pitch) = nTeeth * Circular_Pitch / 360;

function GetPitch_Apex(Gear1_Pitch_r, Gear2_Pitch_r, Axis_a) =
						Gear2_Pitch_r * sin(Axis_a) +
						(Gear2_Pitch_r * cos(Axis_a) + Gear1_Pitch_r) / tan(Axis_a);

function GetCone_Dist(Pitch_Apex, Pitch_r) = sqrt(Pitch_Apex*Pitch_Apex + Pitch_r*Pitch_r);


function GetCone_Distance(nGear1Teeth, nGear2Teeth, Circular_Pitch, axis_angle) =
					GetCone_Dist(GetPitch_Apex(GetPitch_Radius(nGear1Teeth, Circular_Pitch), GetPitch_Radius(nGear2Teeth, Circular_Pitch),axis_angle),
						GetPitch_Radius(nGear1Teeth, Circular_Pitch));

function GetPitch_Apex2(cone_distance, Gear2_Pitch_r) = sqrt(cone_distance*cone_distance - Gear2_Pitch_r*Gear2_Pitch_r);

function GetPitch_a(Pitch_r, Cone_Dist) = asin(Pitch_r/Cone_Dist);
...
module bevel_gear_pair(gear1_teeth = 41,
	...
	pitch_apex1=GetPitch_Apex(GetPitch_Radius(gear1_teeth, outside_circular_pitch), GetPitch_Radius(gear2_teeth, outside_circular_pitch), axis_angle);
							
	cone_distance = GetCone_Distance(gear1_teeth, gear2_teeth, outside_circular_pitch, axis_angle);
							
	pitch_apex2 = GetPitch_Apex2(cone_distance, GetPitch_Radius(gear2_teeth, outside_circular_pitch));
	
	pitch_angle1 = GetPitch_a(GetPitch_Radius(gear1_teeth, outside_circular_pitch), cone_distance);
	pitch_angle2 = GetPitch_a(GetPitch_Radius(gear2_teeth, outside_circular_pitch), cone_distance);
					
	echo(str("cone_distance = ", cone_distance));	
	echo(str("pitch_angle1 = ",pitch_angle1, ", pitch_angle2 = ", pitch_angle2));
	...
	rotate([0,0,90]) {
		...
		translate([0,0,-pitch_apex1])
			bevel_gear(number_of_teeth=gear1_teeth,
						cone_distance=cone_distance,face_width=8,
		...
		rotate([0,-(pitch_angle1+pitch_angle2),0])
			translate([0,0,-pitch_apex2])
				bevel_gear(number_of_teeth=gear2_teeth,
							cone_distance=cone_distance,face_width=8,
							outside_circular_pitch=outside_circular_pitch,
							pressure_angle=22,
							clearance = 0.2,
...
// bevel_gear_pair(gear1_teeth = 36, gear2_teeth = 24, axis_angle = 90, outside_circular_pitch=300);

//Bevel Gear Finishing Options:
bevel_gear_flat = 0;
bevel_gear_back_cone = 1;

module bevel_gear(number_of_teeth=11,
					cone_distance=100,
					face_width=20,
					outside_circular_pitch=1000,
					pressure_angle=30,
	...
	echo(str("bevel_gear: teeth = ", number_of_teeth, ", cone distance = ", cone_distance));
	echo(str("  Face = ",	face_width, ", Pitch = ",outside_circular_pitch));
	echo(str("  Pressure Angle = ",	pressure_angle, ", Clearance = ", clearance));
	echo(str("  Bore = ", bore_diameter, ", Facets = ", involute_facets, ", Finish = ", finish));
	echo(str("  Pitch Angle = ", pitch_angle));
	...
	back_cone_end_radius = outside_pitch_radius -
	...
	face_cone_end_radius = outside_pitch_radius -
	...
	difference(){
		intersection(){
			union(){
				rotate (half_thick_angle)
					translate ([0,0,pitch_apex-apex_to_apex])
						cylinder (r1=root_cone_full_radius, r2=0, h=apex_to_apex, $fn=number_of_teeth*2);
				
				for (i = [1:number_of_teeth])
					rotate ([0,0,i*360/number_of_teeth]) hull()
			...
			if (finish == bevel_gear_back_cone){				
				translate([0,0,-back_cone_descent])
					cylinder(r1=back_cone_end_radius, r2=back_cone_full_radius*2,
							h=apex_to_apex + back_cone_descent, $fn=number_of_teeth*2);
			} else {
				translate([-1.5*outside_pitch_radius, -1.5*outside_pitch_radius, 0])
					cube([3*outside_pitch_radius, 3*outside_pitch_radius, bevel_gear_flat_height]);
			}
		...
		if (finish == bevel_gear_back_cone)
			translate([0,0,-face_cone_descent])
				cylinder(r1=face_cone_end_radius,
		...
		if (bore_diameter>0)
		translate([0,0,pitch_apex - apex_to_apex])
			cylinder(d=bore_diameter, h=apex_to_apex);
...
/*
translate([0,18.5,31]) rotate([90,0,0]) rotate([0,0,180/36])
bevel_gear (
	number_of_teeth=36,
...
module involute_bevel_gear_tooth(
	...
	pitch_point = involute(base_radius*2,
...
module gear(
	...
	echo(str("Gear: Teeth = ", number_of_teeth, ", Pitch radius = ", pitch_radius));
	...
	circle_diameter=min(0.70*circle_orbit_curcumference/circles, (rim_radius-hub_diameter/2)*0.9);
...
function rotate_point(rotate, coord) =[cos (rotate) * coord[0] + sin (rotate) * coord[1],
...
module test_gears(){
	translate([17,-15]){
		...
		rotate([0,0,360*4/17]) translate([39.088888,0,0]){
			...
			translate([0,0,8]){
				...
				rotate([0,0,360*5/6]) translate([22.5,0,1])
		...
		rotate([0,0,360*-5/17]) translate([44.444444444,0,0])
			gear (number_of_teeth=15,
				circular_pitch=500,
				hub_diameter=10,
				rim_width=5,
		...
		rotate([0,0,360*-1/17]) translate([30.5555555,0,-1])
...
module meshing_double_helix(){
	...
	mirror([0,1,0]) translate([58.33333333,0,0])
...
module test_backlash(){
	...
	color([0,0,0.5,1]) translate([0,0,-5]) cylinder($fn=20, r=backlash / 4, h=25);
} // test_backlash
