...
module CableRedirect(Tube_OD=PML98Body_OD, Skirt_ID=PML98Body_ID, 
	...
	difference(){
		union(){
			...
			//*
			translate([0,0,-20]) {
				rotate([0,0,-20]) CenteringRing(OD=Tube_ID-IDXtra, ID=InnerTube_OD+IDXtra, 
...
//translate([0,0,50]) Stager_Mech(Tube_OD=PML98Body_OD, nLocks=2, Skirt_ID=PML98Body_ID, Skirt_Len=30, KeyOffset_a=-30);
rotate([0,0,-30]){
...
module CableEndAndStop(Tube_OD=PML98Body_OD){
	...
	difference(){
		union(){
			...
			rotate([0,0,180/nBottomBolts]) translate([0,Race_ID/2+Bolt4Inset*2,3])
...
module Stager2_Mech(Tube_OD=PML98Body_OD, nLocks=2, Skirt_ID=PML98Body_ID, Skirt_Len=30, KeyOffset_a=0){
	// KeyOffset_a must be a multiple 15°
	...
	BallCircle_d=Tube_OD-6-Ball_d; // 3D printed bearing the cable rotates
	CamBall_a=-25.5;
	...
	module BackStop(){
		...
		difference(){
			...
			translate([0,0,-Block_H-Overlap])
				cylinder(d=Tube_OD-StagerLockInset_Y*2-LockBall_d, h=Block_H+Overlap*2);
				
			BallPath();
			mirror([1,0,0]) BallPath();
			
			translate([0,0,-20]) Stager_LockRod_Holes(Tube_OD=Tube_OD, nLocks=nLocks);
