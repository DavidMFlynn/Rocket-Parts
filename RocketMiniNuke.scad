...
// MN_PetalHub();
// rotate([180,0,0]) PD_Petals(OD=BT75Coupler_OD, Len=Petal_Len, nPetals=3, Wall_t=1.8, AntiClimber_h=4, HasLocks=false, Lock_Span_a=0);
...
// rotate([180,0,0]) MPL_InlineLockRing();
// MPL_InlineServoRing();
...
LowerPodCoupler_Len=108;
echo(LowerPodCoupler_Len=LowerPodCoupler_Len);
MotorPod_OD=BT75Coupler_OD;
MotorPodBody_OD=BT75Body_OD;
MotorPodBody_ID=BT75Body_ID;

LowerPodBody_Len=90;
echo(LowerPodBody_Len=LowerPodBody_Len);
UpperPodBody_Len=181;
...
module ShowRocket(ShowInternals=false){
	Motor_Z=60;
	Fin_Z=45;
	CenterLine_Z=Fin_Z+FinLength+Body_Ext/2+102;
	LockRing_Z=CenterLine_Z-(Body_OD/2+Body_Ext/2)*AftPointy+25+LowerPodCoupler_Len;
	...
	//*
	if (ShowInternals)
		translate([0,0,LockRing_Z-7]) //rotate([0,0,180]) 
	...
	translate([0,0,CenterLine_Z]){
		...
		// Thrust ring
		if (ShowInternals)
			translate([0,0,-(Body_OD/2+Body_Ext/2)*AftPointy+15]) color("LightBlue") 
		...
		// Lower Pod Body tube
		if (ShowInternals)
			translate([0,0,-(Body_OD/2+Body_Ext/2)*AftPointy+25]) color("LightBlue") 
...
module ForwardDome(OD=Body_OD, Extension=Body_Ext/2){
	...
	difference(){
		...
		mirror([0,0,1]) cylinder(d=OD+2, h=OD/2+1);
	} // difference
} // ForwardDome
...
module ForwardShell(){	
	...
	nNoseBolts=8;
	NoseBolt_a=48;
	NoseBolt_Z=Tip_Z-43*Pointy;
	NoseBolt_K=71;
	NoseBolt_d=12;
	
	nRow2Bolts=8;
	Row2Bolt_a=66;
	Row2Bolt_Z=Tip_Z-59*Pointy;
	Row2Bolt_K=85;
	Row2Bolt_d=14;
	
	nRow3Bolts=6;
	Row3Bolt_a=86;
	Row3Bolt_Z=Tip_Z-94*Pointy;
	Row3Bolt_K=98.5;
	Row3Bolt_d=18;
	...
	Ring4_Z=Tip_Z-65*Pointy;
	Ring5_Z=Tip_Z-72*Pointy;
	Ring6_Z=Tip_Z-100*Pointy;
	...
	difference(){
		union(){
			...
			// rings
			if (IncludeArt){
				translate([0,0,Ring1_Z]) rotate_extrude() translate([70.5,0,0]) circle(d=3);
				translate([0,0,Ring2_Z]) rotate_extrude() translate([74,0,0]) circle(d=3.5);
				translate([0,0,Ring3_Z]) rotate_extrude() translate([85.5,0,0]) circle(d=3.5);
				translate([0,0,Ring4_Z]) rotate_extrude() translate([94.5,0,0]) circle(d=3.5);
				translate([0,0,Ring5_Z]) scale([1,1,2]) rotate_extrude() translate([96.6,0,0]) circle(d=3.5);
				translate([0,0,Ring6_Z]) scale([1,1,2]) rotate_extrude() translate([Body_OD/2,0,0]) circle(d=3.5);
			}
		} // union
		...
		if (IncludeArt){
			...
			rotate([0,0,22.5]) Row2BoltLocations() cylinder(d=Row2Bolt_d+1, h=5);
	...
	// Decorative bolts
	if (IncludeArt){
		NoseBoltLocations() PanHead(D=NoseBolt_d, H=2.8, Slot_w=2);
		rotate([0,0,22.5]) Row2BoltLocations() PanHead(D=Row2Bolt_d, H=3, Slot_w=2);
		Row3BoltLocations() PanHead(D=Row3Bolt_d, H=3.4, Slot_w=2.5);
	}
	...
	difference(){
		...
		for (j=[0:nShellBolts-1]) rotate([0,0,360/nShellBolts*j]) translate([0,Body_OD/2,-7.5]) rotate([-90,0,0]) Bolt6Hole();
		
		if ($preview) translate([0,0,-20]) cube([Body_OD/2+1, Body_OD/2+1, Body_OD/2+Body_Ext/2+1]);
	} // difference
	
	
	difference(){
		translate([0,0,(Body_OD/2+Body_Ext/2)*Pointy-40]) {
		...
		if ($preview) translate([0,0,(Body_OD/2+Body_Ext/2)*Pointy-50]) cube([Body_OD/2+1, Body_OD/2+1, Body_OD/2+Body_Ext/2+1]);
	} // difference
} // ForwardShell
...
module AftShell(){
	...
	difference(){
		union(){
			...
			for (j=[0:nFins-1]) rotate([0,0,360/nFins*j]) translate([-15,45,MotorStop_Z]) rotate([15,0,0]) cube([30,36,36]);
		...
		// cut off base
		translate([0,0,Base_Z]) cylinder(d=Body_OD, h=MotorStop_Z-Base_Z);
		
		// Pod tube
		translate([0,0,Base_Z])
			cylinder(d=MotorPodBody_OD+IDXtra*2, h=50, $fn=$preview? 90:360);
			
		// Shell bolts
		for (j=[0:nShellBolts-1]) rotate([0,0,360/nShellBolts*j]) translate([0,Body_OD/2,-7.5]) rotate([-90,0,0]) Bolt6ButtonHeadHole();
	...
	difference(){
		translate([0,0,MotorStop_Z]) Tube(OD=MotorPodBody_OD+IDXtra*2+4.4, ID=MotorPodBody_OD+IDXtra*2, Len=75, myfn=$preview? 90:360);
	...
	difference(){
		...
		// Pod tube
		translate([0,0,Base_Z])
			cylinder(d=MotorPodBody_OD+IDXtra*2, h=250, $fn=$preview? 90:360);
...
module TheButt(){
	...
	difference(){
		...
		difference(){
			translate([0,0,MotorStop_Z]) cylinder(d=Body_OD+2, h=Body_OD);
			
			translate([0,0,MotorStop_Z-Overlap]) cylinder(d=Coupler_ID, h=30);
		} // difference
		
		// Motor Pod tube and stop ring
		translate([0,0,MotorStop_Z-StopRing_Len])
...
module MN_PetalHub(){
	...
	difference(){
		union(){
			...
			// Skirt			
			translate([0,0,-15]) Tube(OD=Coupler_OD, ID=Coupler_OD-4.4, Len=15+Overlap, myfn=$preview? 90:360);
		} // union
...
module FlatFinSocket(){
	hull(){
		// Root
		translate([-FinTip_W/2-IDXtra, MotorPodBody_OD/2+5-IDXtra, -FinLength+80])  cube([FinTip_W+IDXtra*2,10+IDXtra*2,FinRoot_L-55]);
		// Tip
		translate([-FinTip_W/2-IDXtra, Body_OD/2-RingFin_Thickness, -FinLength+25]) cube([FinTip_W+IDXtra*2,Overlap,FinTip_L]);
	} // hull
...
module RingFin(){
	...
	difference(){
		union(){
			...
			translate([0,Body_OD/2,-32]) hull(){
			rotate([90,0,0]) cylinder(d=20, h=3);
			translate([0,0,22.7]) rotate([90,0,0]) cylinder(d=20, h=3);
		}
		} // union
		
		for (j=[0:nFins-1]) rotate([0,0,360/nFins*(j+0.5)]) translate([0,0,-RingFin_Len]) hull(){
				rotate([0,0,20]) rotate([90,0,0]) RoundRect(X=3, Y=70, Z=Body_OD/2+20, R=3);
				rotate([0,0,-20]) rotate([90,0,0]) RoundRect(X=3, Y=70, Z=Body_OD/2+20, R=3);
			}
		...
		translate([0,Body_OD/2,-27]) {
			rotate([-90,0,0]) Bolt6Hole();
			translate([0,0,12.7]) rotate([-90,0,0]) Bolt6Hole();
		}
	} // difference
} // RingFin
