...
module BC_Cup(Tube_OD=120, ID=90, nLocks=3){
	...
	difference(){
		union(){
			...
			translate([0,0,-2]) cylinder(d1=Tube_OD-8, d2=Tube_OD-4, h=2+Overlap);
		} // union
		
		translate([0,0,-2-Overlap]) cylinder(d=ID, h=Len+2+Overlap*2);
		for (j=[0:nLocks-1]) rotate([0,0,360/nLocks*j]) translate([0,Tube_OD/2-LR_Y/2-5,-2-Overlap])
			RoundRect(X=LR_X+IDXtra*2, Y=LR_Y+IDXtra*2, Z=LR_Z, R=1);
	} // difference
	
	if ($preview)
		for (j=[0:nLocks-1]) rotate([0,0,360/nLocks*j]) translate([0,Tube_OD/2-LR_Y/2-5,-16])
...
module BC_Saucer(Tube_OD=120, nLocks=3){
	...
	difference(){
		translate([0,0,-Len]) cylinder(d=Tube_OD, h=Len);
		
		translate([0,0,-2]) cylinder(d1=Tube_OD-8-IDXtra, d2=Tube_OD-4-IDXtra, h=2+Overlap);
		
		translate([0,0,-Len-Overlap]) cylinder(d=ID, h=Len);
		
		for (j=[0:nLocks-1]) rotate([0,0,360/nLocks*j]) translate([0,Tube_OD/2-LR_Y/2-5,-Len-Overlap])
			RoundRect(X=LR_X+IDXtra*2, Y=LR_Y+IDXtra*2, Z=LR_Z, R=1);
	} // difference
	
	for (j=[0:nLocks-1]) rotate([0,0,360/nLocks*j]) {
	translate([LR_X/2-2+CP_Bearing_OD+Arm_Len,Tube_OD/2-LR_Y/2-5-5,-Len-20]) cube([10,10,20]);
	translate([-LR_X/2+2-CP_Bearing_OD-Arm_Len-10,Tube_OD/2-LR_Y/2-5-5,-Len-20]) cube([10,10,20]);
	}
	
	if ($preview)
		for (j=[0:nLocks-1]) rotate([0,0,360/nLocks*j]) translate([0,Tube_OD/2-LR_Y/2-5,-Len-CP_Bearing_OD/2]){
			...
			translate([LR_X/2+CP_Bearing_OD/2+Overlap,0,0]) {
				ShowBearing();
				rotate([0,Disengage_a,0]) translate([Arm_Len,0,0]) ShowBearing();
			}
